...
| `instanceColumns` | []string | `[name, status, type, ipv4, ipv6, snapshots]` | Which columns the Instances panel shows, and in what order. Valid values: `name`, `status`, `type`, `ipv4`, `ipv6`, `project`, `service`, `snapshots`. `project` is added automatically whenever the list spans more than one project. Unknown values are ignored; list any subset to hide the rest. `service` is off by default: it shows the [incus-compose](https://github.com/lxc/incus-compose) service an instance came from, and is blank for instances created any other way. |
