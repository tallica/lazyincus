...
| `instanceColumns` | []string | `[name, status, type, ipv4, ipv6, snapshots]` | Which columns the Instances panel shows, and in what order. Valid values: `name`, `status`, `type`, `ipv4`, `ipv6`, `project`, `service`, `health`, `image`, `snapshots`. `project` is added automatically whenever the list spans more than one project. Unknown values are ignored; list any subset to hide the rest. `service`, `health` and `image` are off by default and read [incus-compose](https://github.com/lxc/incus-compose) config keys, so all three are blank for instances created any other way: the service an instance came from, its `ic-healthd` health (`starting`, `healthy`, `unhealthy`, `stopped`, or `unknown` for a service that declared no healthcheck), and the image reference the compose file named. |
