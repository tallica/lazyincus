...
- Sixth side panel, **Compose** (`6`), listing incus-compose's managed projects with `u`/`d` to start/stop the local one — see [CLAUDE.md](CLAUDE.md#compose). The compose config/credits/aggregate-log tabs from BACKLOG.md's [project panel](BACKLOG.md#3-project-panel) plan are still deferred.
